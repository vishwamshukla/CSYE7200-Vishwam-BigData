...
object WebCrawler extends App {
  ...
  def wget(url: URL)(implicit ec: ExecutionContext): Future[Seq[URL]] = {
    ...
    def getURLs(html: String): Seq[Try[URL]] = {
      // Regular expression pattern to match URLs in anchor tags
      val anchorPattern = """<a\s+(?:[^>]*?\s+)?href="([^"]*)"""".r

      // Find all matches of anchor tags with href attribute
      val matches = anchorPattern.findAllMatchIn(html)

      // Extract URLs from the matches and convert them to URLs
      val urls = matches.flatMap { matchResult =>
        val urlStr = matchResult.group(1)
        Try(new URL(urlStr)).toOption // Convert Try to Option to filter out failed attempts
      }

      // Return the sequence of Try[URL]
      urls.map(Success(_)).toSeq
    }
    ...
    def getLinks(g: String): Try[Seq[URL]] = {
      ...
      for {
        ...
        htmlString = n.toString
        // Pass the String to getURLs
        uys = getURLs(htmlString)
        us <- MonadOps.sequenceForgiveSubsequent(uys) {
    ...
    for {
      ...
      links <- Future.fromTry(linksTry) // Convert Try to Future
    } yield links
  }
